...
#    fidx = (freq>(N/2))
#    freq[fidx] -= N
#    freq += N*np.floor(-(freq/N - f0/fp) + 0.5).astype(int)
#    #freq += N*np.round(f0/fp).astype(int)
#    #center_idx = (N*np.mod(f0,fp)).astype(int)
#    #freq += center_idx
#    return freq*fp/N
#
#def new_FFT_freq(N, fp, f0):
#    bands = np.arange(np.round(f0/fp) - 2, np.round(f0/fp) + 3)
#    freq = np.array([np.arange(N)*fp/N + fp*b for b in bands])
#    weight = np.exp(-np.abs(freq - f0))
#    max_idx = np.argmax(weight, axis=0)
#    return np.array([freq[midx, k] for midx,k in zip(max_idx,range(N))])
...
def twoWayArrayPatternTrueTimeFlat(fastTime, lookDirections, ranges, rad, Ztx = None, Zrx = None, FFTLen = 16384):
    ...
    # Get the near range
    nearRange = rad['acquisition']['nearRangeTime']*physical.c/2.0
    ...
    # Compute the fast time frequency values
    w = 2.0*np.pi*FFT_freq(len(fastTime), 1.0/rad['acquisition']['rangeSampleSpacing'], physical.c/wavelength)
    #w = 2.0*np.pi*FFT_freq(len(fastTime), 1.0/ref['rangeSampleSpacing'], 0.0) + 2.0*np.pi*physical.c/wavelength
    W = np.matlib.repmat(w, len(lookDirections), 1).T
    ...
    # Compute the azimuth component of the signal
    Az = np.exp(-1j*2.0/physical.c*np.matlib.repmat(ranges - nearRange,len(w),1)*W)
    ...
    if Ztx is None:
        ...
        Ztx = np.zeros(Z.shape, dtype=Z.dtype)
        idx_c = (Z==1.0)
        idx = (idx_c == False)
        Ztx[idx] = (Z[idx]**tx1idx[0][0])*(1.0-Z[idx]**tx1len)/(1.0-Z[idx])
        Ztx[idx_c] = tx1len
        Z = None
    if Zrx is None:
        rx1idx = np.where(rx>0.0)
        (rx1len,) = rx1idx[0].shape
        Z = np.exp(-1j*d/physical.c*W*(U-rxuZero))
        Zrx = np.zeros(Z.shape, dtype=Z.dtype)
        idx_c = (Z==1.0)
        idx = (idx_c == False)
        Zrx[idx] = (Z[idx]**rx1idx[0][0])*(1.0-Z[idx]**rx1len)/(1.0-Z[idx])
        Zrx[idx_c] = rx1len
        Z = None
    print(tx.shape)
    print(U.shape)
    ...
    # Compute the chirp and transform into frequency domain
    chirp = chirpWaveform(rad['chirp']['pulseBandwidth'], rad['chirp']['length'])
    S = np.matlib.repmat(np.fft.fft(chirp.sample(fastTime - rad['acquisition']['nearRangeTime'], 
                                                 carrier=rad['antenna']['fc'])), len(ranges),1).T
    #D = np.exp(1j*W*rad['acquisition']['nearRangeTime'])
    ...
    return np.fft.ifft(Z, axis=0)/len(fastTime), Ztx, Zrx
...
#%% Array pattern function (FUNDAMENTAL)
def twoWayArrayPatternLinearKSOld(ks, rad, krz, dS = 0.0):
    kShape = ks.shape
    ...
    # Compute the array factor
    s_positions = dS + rad['antenna']['azimuthPositions']
    tx_weight = rad['mode']['txMagnitude']
    rx_weight = rad['mode']['rxMagnitude']
    [S, KS] = np.meshgrid(s_positions, ks)
    [Atx, KS] = np.meshgrid(tx_weight, ks)
    [Arx, KS] = np.meshgrid(rx_weight, ks)
    
    aF = (np.sum(Atx*np.exp(1j*s_positions/2.0*(KS - rad['mode']['txuZero']*krz)), axis = 1) 
    * np.sum(Arx*np.exp(1j*s_positions/2.0*(KS - rad['mode']['rxuZero']*krz)), axis = 1))
    
    return np.reshape(aF*eF, kShape)
...
def computeSignal(radar, pointXYZ, satSV): 
    ...
    for rad in radar:
        ...
        azimuthPositions = rad['antenna']['azimuthPositions']/physical.c
...
def multiChannelProcess(radar, bands=None, p=0.5, make_plots=False, SNR = 1.0):
    ...
    return procData[r_sys.ksidx[r_sys.ks_full_idx],:], r_sys
