...
#%%
PODFile = r"C:\Users\Ishuwa.Sikaneta\local\sarsim\radar\orbit\S1A_OPER_AUX_PROQUA_POD__20211005T022735_V20211002T235942_20211003T235941.DBL"


#%% POD attitude file
class attitude_S1POD(attitude):
    """
    Class to load attitude data from a POD file
    
    Methods
    -------
    readAttitude:
        Reas state vectors from a POD file
    
    """
    def readMeasurements(self, PODFile):
        with open(PODFile) as f:
            attData = [l.split() for l in f.read().split('\n') if '#' not in l
                       and l]
            
        self.measurementTime = [np.datetime64("T".join(
            [att[0].replace('/', '-'), att[1]])) for att in attData]
        
        self.qData = [np.quaternion(*[float(a) for a in att[2:6]]) 
                      for att in attData]
...
roll, pitch, yaw = np.radians(at.eData[0])
